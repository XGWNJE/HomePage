...
const HOST = process.env.INDEXNOW_HOST || 'xgwnje.cn';
...
async function main() {
	...
	if (args.length === 0) {
		...
		console.log('   or: node scripts/indexnow.mjs https://xgwnje.cn/blog/new-post/');
		process.exit(0);
	}
	...
	if (args[0] === 'sitemap') {
		...
		urls = await fetchUrlsFromSitemap(`https://xgwnje.cn/sitemap.xml`);
		console.log(`   Found ${urls.length} URLs`);
	} else {
		// Use provided URLs
		urls = args.map((url) => {
			// Ensure URL has protocol
			return url.startsWith('http') ? url : `https://xgwnje.cn${url.startsWith('/') ? '' : '/'}${url}`;
		});
	}
