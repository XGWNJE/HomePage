...
- 网页发表通道：后台 `/admin/editor/` 提供文章编辑器（草稿存 SQLite `article_drafts` 表，30 秒自动保存，右侧为 marked 近似预览）；发表走 `POST /api/admin/article/publish` → 服务器端 site-release 构建上线，成功后删除草稿，失败保留草稿。第一期仅支持中文稿（slug 必须 `-cn` 结尾），正文引用的 `/uploads/` 图片会在发表时复制为 `public/image/blog/<group>/` 文章专属资产并改写 URL。预览渲染与 Astro 内容管线不逐字一致，交付时须声明。
