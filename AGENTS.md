...
- 经过人工批准的 Journal 文章源记录位于 `src/content/human-agency/`，由组合 blog loader 映射为普通博客文章并标记为“消化”。仓库外 Journal 包只能在显式 `JOURNAL_PREVIEW_PACKAGE` 下预览；正式导入只接受 `approved-publish`，默认 dry-run，写入必须显式 `--apply`。原始观察、私有 claim、绝对路径和审核事件不得进入本仓库。
