...
- VPS 上存在服务器端文章发布地基：`/opt/homepage-site` 是专用发布克隆（deploy key 仅限本仓库读写），`/opt/node22` 是前端构建用的独立 Node 22，`server/scripts/site-release.mjs` 在其上完成"写入文章 → 构建 → 版本化原子切换"。它与本地发布通道共用 flock 和 releases 结构，不得绕过脚本直接改动该克隆或线上静态目录。API 仅可通过 sudoers 白名单中的发布脚本与 `homepage-site-sync` 两条命令以 root 调用该地基。
