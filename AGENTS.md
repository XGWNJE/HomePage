...
- **前端改动预览**：每次前端改动完成后，主动开本地预览供用户观察，统一用 `npm run preview:admin`（同时起本地前后端，自动以专用验收管理员账号 `preview-admin` 登录并打开浏览器）；用户表示验收通过（如"通过验收"）即视为授权，直接关闭预览；转向其他话题等其余情况仅提醒预览仍在运行、会占用资源，不擅自关闭。
