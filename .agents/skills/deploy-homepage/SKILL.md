...
- 默认只发布实际变更的部分。仅包含 `src/content/blog/**` 与 `public/image/blog/**` 的文章变更使用 `ContentOnly`；其他纯静态前端变更使用 `FastFrontend`。不得为了沿用旧流程顺带发布未变更的 `server/`。
...
该命令自动完成内容配对、图片预算、生产构建、变更范围拒绝、制品/manifest/SHA-256、上传、原子切换、受影响路由验收、失败回滚与 `Server-infra AfterChange`。只做发布计划与本地构建时运行 `npm run content:release:plan`。文章发布仍构建完整静态站，因为首页、列表、标签、RSS 与 Sitemap 必须与文章保持同一版本；它不会部署 API、修改 Nginx 或执行全站浏览器矩阵。
...
- 生成 UTC/本地一致的 `release_id`，只打包本次发布的部分，记录文件数、字节数和 SHA-256；再对制品、影响路由与构建环境摘要的 release manifest 计算 SHA-256。
...
严格按发布契约中的当前档位执行。每一步先上传到新路径并验证，再切换 `current`/正式目录；版本化制品、SHA-256、备份、原子切换、回滚入口和 `Server-infra AfterChange` 在两个档位都不可省略。只有发布后端时才执行数据库、上传目录和环境文件备份。

## 5. 验收与交付

- `ContentOnly` 检查文章页、首页、博客、标签、RSS、Sitemap、API health 与 404；只在文章包含自定义交互或布局时追加浏览器验收。
